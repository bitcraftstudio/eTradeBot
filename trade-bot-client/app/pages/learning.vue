<template>
	<UDashboardPanel id="learning">
		<template #header>
			<Topbar :title="'Learning'" :description="'Performance insights and patterns'" />
		</template>
		<template #body>
			<div class="space-y-6">
				<!-- Run Learning Review -->
				<UCard>
					<template #header>
						<div class="flex items-center justify-between">
							...
							<UButton :loading="running" @click="runLearningReview">
								Run Learning Review
							</UButton>
						</div>
					</template>
					...
					<div class="space-y-6">
						...
						<div v-if="latestInsight.aiReflection" class="p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20">
							<div class="flex items-start gap-3">
								...
								<div class="flex-1">
									<p class="text-sm font-medium text-blue-900 dark:text-blue-300 mb-2">AI Reflection
									</p>
						...
						<div v-if="latestInsight.successFactors?.length > 0">
							...
							<div class="space-y-2">
								<div v-for="(factor, idx) in latestInsight.successFactors" :key="idx"
						...
						<div v-if="latestInsight.failureFactors?.length > 0">
							...
							<div class="space-y-2">
								<div v-for="(factor, idx) in latestInsight.failureFactors" :key="idx"
						...
						<div v-if="latestInsight.patterns?.length > 0">
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div v-for="pattern in latestInsight.patterns.slice(0, 6)" :key="pattern.pattern"
									class="p-4 rounded-lg bg-gray-50 dark:bg-gray-800">
									<div class="flex items-start justify-between mb-2">
										<p class="font-medium text-gray-900 dark:text-white text-sm">
											{{ formatPatternName(pattern.pattern) }}
										</p>
										<UBadge :color="getConfidenceColor(pattern.confidence)" variant="subtle" size="sm">
											{{ pattern.confidence }}
										</UBadge>
									</div>
