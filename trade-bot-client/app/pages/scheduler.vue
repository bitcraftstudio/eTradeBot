<template>
	<UDashboardPanel id="scheduler">
		<template #header>
			<Topbar :title="'Scheduler'" :description="'Automation and scheduled tasks'" />
		</template>
		<template #body>
			<div class="space-y-6">
				...
				<UCard>
					...
					<div class="space-y-6">
							...
							<div class="flex items-start justify-between">
								<div class="flex-1">
									<div class="flex items-center gap-2 mb-2">
										<UIcon :name="config.autoTradeEnabled ? 'i-heroicons-bolt' : 'i-heroicons-bolt-slash'"
											class="w-5 h-5"
											:class="config.autoTradeEnabled ? 'text-red-600 dark:text-red-400' : 'text-gray-400'" />
										<h4 class="font-semibold"
											:class="config.autoTradeEnabled ? 'text-red-900 dark:text-red-100' : 'text-gray-900 dark:text-white'">
											Automatic Trading
										</h4>
									</div>
									<p class="text-sm"
										:class="config.autoTradeEnabled ? 'text-red-700 dark:text-red-300' : 'text-gray-600 dark:text-gray-400'">
										{{ config.autoTradeEnabled
						...
						<UFormGroup label="Watchlist" :hint="`${watchlist.length} symbols`">
							<div class="space-y-3">
								<div class="flex gap-2">
									...
									<UButton v-if="watchlist.length > 0" color="error" variant="soft" @click="clearWatchlist">
										Clear All
									</UButton>
								</div>
								...
								<div v-if="watchlist.length > 0" class="flex flex-wrap gap-2">
									<div v-for="(symbol, index) in watchlist" :key="`${symbol}-${index}`"
						...
						<UAlert v-if="hasUnsavedChanges" icon="i-heroicons-exclamation-triangle" color="yellow" variant="soft"
							title="Unsaved Changes"
							description="You have unsaved changes. Click 'Save Configuration' to persist them." />

						<!-- Save Button -->
						<div class="flex gap-3">
							<UButton class="flex-1" size="lg" :loading="saving" :color="hasUnsavedChanges ? 'primary' : 'gray'"
								@click="saveConfig">
								{{ hasUnsavedChanges ? 'Save Configuration' : 'Configuration Saved' }}
							</UButton>
							<UButton size="lg" color="error" variant="outline" :loading="resetting" @click="resetConfig">
								Reset to Defaults
							</UButton>
						</div>
					</div>
				...
				<UCard>
					<template #header>
						<div class="flex items-center justify-between">
							...
							<UButton :loading="scanning" @click="triggerScan">
					...
					<div v-if="scanResults" class="mt-6 space-y-4">
						...
						<UTable v-if="scanResults.recommendations?.length > 0" :rows="scanResults.recommendations" :columns="[
							...
							<template #recommendation-data="{ row }">
								<UBadge :color="getRecommendationColor(row.recommendation)" variant="subtle">
									{{ row.recommendation }}
								</UBadge>
							</template>
						...
						<!-- Errors -->
						<UAlert v-if="scanResults.errors?.length > 0" icon="i-heroicons-exclamation-triangle" color="red"
							title="Errors During Scan" :description="`${scanResults.errors.length} errors occurred`" />
					</div>
				</UCard>
			</div>
		</template>
