<template>
	<UDashboardPanel id="trades">
		<template #header>
			<Topbar :title="'Trade History'" :description="'Complete trade history'" />
		</template>
		<template #body>
			<div class="space-y-6">
				<!-- Filters -->
				<UCard>
					<div class="flex flex-wrap gap-4">
						<USelectMenu v-model="filter" :options="filterOptions" placeholder="Filter by status" class="w-48" />
						<UInput v-model="search" icon="i-heroicons-magnifying-glass" placeholder="Search symbol..." class="w-64" />
					</div>
				</UCard>
				...
				<UCard>
					...
					<UTable v-if="filteredTrades.length > 0" :rows="filteredTrades" :columns="columns">
						...
						<template #status-data="{ row }">
							<UBadge :color="row.status === 'CLOSED' ? 'neutral' : row.status === 'OPEN' ? 'primary' : 'warning'"
								variant="subtle">
								{{ row.status }}
							</UBadge>
						</template>
						...
						<template #actions-data="{ row }">
							<UButton icon="i-heroicons-eye" color="neutral" variant="ghost" size="sm" @click="viewTradeDetails(row)" />
						</template>
					</UTable>

					<UAlert v-else icon="i-heroicons-information-circle" color="neutral" variant="subtle" title="No trades found"
