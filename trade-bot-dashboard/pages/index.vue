  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <UCard>
        <div class="flex items-start justify-between">
          ...
          <UIcon name="i-heroicons-banknotes-20-solid" class="w-8 h-8 text-blue-500" />
        </div>
      </UCard>
      ...
      <UCard>
        <div class="flex items-start justify-between">
          ...
          <UIcon name="i-heroicons-chart-bar-20-solid" class="w-8 h-8 text-green-500" />
        </div>
      </UCard>
      ...
      <UCard>
        <div class="flex items-start justify-between">
          <div>
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Open Positions</p>
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-2">
              {{ portfolio?.openPositions || 0 }}
            </p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Value: {{ formatCurrency(portfolio?.positionValue || 0) }}
            </p>
          </div>
          <UIcon name="i-heroicons-briefcase-20-solid" class="w-8 h-8 text-purple-500" />
        </div>
      </UCard>
      ...
      <UCard>
        <div class="flex items-start justify-between">
          ...
          <UIcon name="i-heroicons-trophy-20-solid" class="w-8 h-8 text-yellow-500" />
        </div>
      </UCard>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-3">
          ...
          >
            ...
            <div class="text-right">
              <UBadge
                :color="trade.status === 'CLOSED' ? 'gray' : 'green'"
                variant="subtle"
              >
                {{ trade.status }}
              </UBadge>
          ...
          <UAlert
            v-if="!recentTrades || recentTrades.length === 0"
            icon="i-heroicons-information-circle"
            color="gray"
            variant="subtle"
            title="No trades yet"
            description="Start trading to see results here!"
          />
        </div>
      </UCard>
      ...
      <UCard>
        ...
        <div class="space-y-3">
          ...
          <UAlert
            v-if="!activePositions || activePositions.length === 0"
            icon="i-heroicons-information-circle"
            color="gray"
            variant="subtle"
    ...
    <UCard v-if="latestInsight">
      ...
      <div class="space-y-4">
        ...
        <UAlert
          v-if="latestInsight.aiReflection"
          icon="i-heroicons-light-bulb"
          color="primary"
          variant="subtle"
          :title="'AI Reflection'"
          :description="latestInsight.aiReflection"
        />
      </div>
    </UCard>
  </div>
