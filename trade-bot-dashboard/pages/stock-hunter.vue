      ...
      <template #header>
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <UBadge
              :color="status?.apiConfigured ? 'green' : 'yellow'"
              variant="subtle"
            >
      ...
      <div class="space-y-6">
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <UFormGroup label="Minimum">
              <UInput
                v-model.number="filters.minSmartScore"
                type="number"
                min="1"
                max="10"
              />
            ...
            <UFormGroup label="Maximum">
              <UInput
                v-model.number="filters.maxSmartScore"
                type="number"
                min="1"
                max="10"
              />
          ...
          <div class="mt-2 flex gap-2">
            <UBadge @click="filters.minSmartScore = 10; filters.maxSmartScore = 10" class="cursor-pointer" color="green">Perfect (10)</UBadge>
            <UBadge @click="filters.minSmartScore = 9; filters.maxSmartScore = 10" class="cursor-pointer" color="primary">Excellent (9-10)</UBadge>
            <UBadge @click="filters.minSmartScore = 8; filters.maxSmartScore = 10" class="cursor-pointer" color="blue">Good+ (8-10)</UBadge>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <UFormGroup label="Minimum Upside (%)" hint="Minimum % to analyst price target">
            <UInput
              v-model.number="filters.minUpside"
              type="number"
              min="0"
              max="100"
              placeholder="5"
          ...
          <UFormGroup label="Minimum Market Cap ($B)" hint="Minimum company size">
            <UInput
              v-model.number="filters.minMarketCapB"
              type="number"
              min="0"
              placeholder="1"
            />
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <UFormGroup label="Min Analyst Rating" hint="1-5 scale (4 = Buy)">
              <UInput
                v-model.number="filters.minAnalystRating"
                type="number"
                min="1"
                max="5"
                step="0.1"
    ...
    <div v-if="huntResults?.recommendations?.length > 0" class="space-y-4">
      ...
      <UCard v-if="huntResults.summary?.topPick" class="border-2 border-primary-500">
        ...
        <div class="mt-4 flex gap-2">
          <UButton
            color="primary"
            @click="addToWatchlist(huntResults.summary.topPick.symbol)"
          >
            Add to Watchlist
          </UButton>
          <UButton
            color="gray"
            variant="outline"
            @click="viewDetails(huntResults.summary.topPick)"
          >
            View Details
          </UButton>
        </div>
      ...
      <UCard>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div
            v-for="stock in huntResults.recommendations"
            :key="stock.symbol"
          ...
          >
            ...
            <div class="flex items-start justify-between mb-3">
              ...
              <UBadge
                :color="getSmartScoreColor(stock.smartScore)"
                size="lg"
              >
                {{ stock.smartScore }}
              </UBadge>
            ...
            <div class="flex flex-wrap gap-1 mb-3">
              <UBadge
                v-if="stock.hedgeFundTrend === 'Increasing'"
                color="blue"
                variant="subtle"
                size="xs"
              >
              ...
              <UBadge
                v-if="stock.insiderSentiment === 'Positive'"
                color="purple"
                variant="subtle"
                size="xs"
              >
              ...
              <UBadge
                v-if="stock.analystConsensus.buy > 20"
                color="green"
                variant="subtle"
                size="xs"
              >
      ...
      <UCard v-if="selectedStock">
        ...
        <div class="space-y-6">
          ...
          <div v-if="selectedStock.reasons?.length > 0">
            ...
            <div class="space-y-2">
              <div
                v-for="(reason, idx) in selectedStock.reasons"
                :key="idx"
                class="flex items-start gap-2 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20"
              >
                <UIcon name="i-heroicons-check-circle" class="w-5 h-5 text-blue-600 dark:text-blue-400 mt-0.5 flex-shrink-0" />
          ...
          <div class="flex gap-2">
            <UButton
              block
              @click="addToWatchlist(selectedStock.symbol)"
            >
              Add to Watchlist
            </UButton>
            <UButton
