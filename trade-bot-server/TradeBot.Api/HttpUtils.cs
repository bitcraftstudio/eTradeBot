...
namespace TradeBot.Api
{
    public class HttpUtils
    {
        ...
        public static async Task<HttpResponseData> JsonResponse(HttpRequestData req, object? data)
        {
            if(data == null)
            {
                var res = req.CreateResponse(HttpStatusCode.NotFound);
                res.Headers.Add("Content-Type", "application/json");
                await res.WriteStringAsync(JsonSerializer.Serialize(new { success = false, error = "Not found" }, JsonOutOpts));
                return res;
            } 
            else {
        ...
        public static async Task<HttpResponseData> JsonResponse(HttpRequestData req, bool success)
        {
            if (!success)
            {
                var res = req.CreateResponse(HttpStatusCode.NotFound);
                res.Headers.Add("Content-Type", "application/json");
                await res.WriteStringAsync(JsonSerializer.Serialize(new { success = false, error = "Failed" }, JsonOutOpts));
                return res;
            }
            else
            {
                var res = req.CreateResponse(HttpStatusCode.OK);
                res.Headers.Add("Content-Type", "application/json");
                await res.WriteStringAsync(JsonSerializer.Serialize(new { success = true, error = "Success" }, JsonOutOpts));
                return res;
            }
        }
